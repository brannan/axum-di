...
#[tokio::main]
async fn main() {
    // Repository as trait object for mocking.
    let repo: DynHeroesRepository = Arc::new(HeroesRepository());

    // Top level router with state.
    // See https://docs.rs/axum/latest/axum/middleware/index.html#rewriting-request-uri-in-middleware
    // for more information about the NormalizePathLayer. use axum::ServiceExt is necessary.

    let normalized_app = NormalizePathLayer::trim_trailing_slash().layer(
        Router::new()
            .nest("/heroes", heroes_routes())
            .with_state(repo)
    );

    // Start the server. Note that for brevity, we do not add logging, 
...
///
/// This trait is used to abstract the data access layer. At runtime, the trait 
/// is implemented by the HeroesRepository struct. During unit testing, the 
/// trait is mocked by the MockHeroesRepositoryTrait.
